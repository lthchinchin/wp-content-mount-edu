...
function my_remove_menus()
{
    ...
    is_has_role('lp_teacher') ? remove_menu_page('tools.php') : '';
...
function lp_admin_css($hook) {
    wp_enqueue_style( 'lp_admin_css', get_template_directory_uri() . '/assets/css/lp-admin-style.css', false, '1.0.0' );
}
add_action('admin_enqueue_scripts', 'lp_admin_css');
...
function wpdocs_add_dashboard_widgets() {
    wp_add_dashboard_widget( 'dashboard_widget', 'Hướng dẫn đăng bài', 'dashboard_widget_function' );
}

if(is_has_role('lp_teacher')){
    add_action('wp_dashboard_setup', 'remove_dashboard_widgets' );
    add_action( 'wp_dashboard_setup', 'wpdocs_add_dashboard_widgets' );
}

function dashboard_widget_function( $post, $callback_args ) {
	esc_html_e( "Xin chào, đây là hướng dẫn đăng bài...!", "textdomain" );
    ?>
        <div>
            <ul>
                <li>Sử dụng [showmore].....your content.....[/showmore] để thu gọn nội dung của bạn.</li>
            </ul>
        </div>
    <?php
}