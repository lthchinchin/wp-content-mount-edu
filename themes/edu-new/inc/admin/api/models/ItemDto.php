...
class ItemDto
{
    ...
    public function get_items($request)
    {
        ...
        $limit      = isset($params['limit']) && $params['limit'] ? $params['limit'] : -1;
        ...
        $city       = isset($params['city']) && $params['city'] && $params['city'] != 'Toàn Quốc' ? $params['city'] : '';
        ...
        $taxonomy_slug = 'loai-hinh-bds';
        switch ($post_type) {
            case 'bds-da-ban':
                $taxonomy_slug = 'loai-hinh-bds';
              break;
            case 'bds-cho-thue':
                $taxonomy_slug = 'loai-hinh-cho-thue';
              break;
            case 'du-an':
                $taxonomy_slug = 'loai-hinh-du-an';
              break;
            default:
          }
        ...
        if ('newest' === $orderby) {
            $args = array(
                ...
                'order'            => 'DESC',
                's' => $search_args,
                's_meta_keys' => array('city','district','ward'),
                'meta_query'       => array(
                    'relation'     => 'AND',
                    // array(
                    //     'key'      => 'deadline_post',
        ...
        if ($get_items->have_posts()) :
            while ($get_items->have_posts()) : $get_items->the_post();
                ...
                $category = wp_get_post_terms(get_the_ID(), $taxonomy_slug, array('fields' => 'names'));
                ...
                if($post_type == 'du-an'){
                    $price_show =  get_field('price') ? get_field('price').' triệu/m&sup2' : 'Liên hệ';
                }else{
                    $price_show = varsland_convert_number_price_to_words(get_field('price'));
                }
                ...
                array_push(
                    ...
                    array(
                        ...
                        'isFavorite' => varsland_db_check_favorite(get_the_ID(),  $currentId),
                        'isVip' => get_field('post_type') == 3 ? true : false,
                        'status' => get_field('status'),
                        'thumbnail' => get_the_post_thumbnail_url(get_the_ID(), 'img_360_180'),
