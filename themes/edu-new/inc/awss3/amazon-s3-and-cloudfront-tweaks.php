...
class Amazon_S3_and_CloudFront_Tweaks
{
	...
	function get_setting_endpoint($url_domain = false)
	{
		if ($url_domain) {
			$minio_url_domain =  str_replace("http://", "", AS3CF_SETTINGS["minio_url_domain"]);
			$minio_url_domain =  str_replace("https://", "", $minio_url_domain);
			return $minio_url_domain . "/";
		}

		return AS3CF_SETTINGS["minio_endpoint"]; //'http://192.168.1.26:9000';
	}
	...
	function minio_s3_client_args($args)
	{
		...
		$args['endpoint'] = $this->get_setting_endpoint();
	...
	function minio_s3_url_domain($domain, $bucket, $region, $expires, $args)
	{
		...
		return $this->get_setting_endpoint(true) . $bucket;
	}
